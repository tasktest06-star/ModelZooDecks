"""AM68A inference engine (8 TOPS C7x dual-DSP + MMA, 8 GB RAM)."""
import sys
import time
...
SUPPORTED_MODELS = {
    "yolox_m_lite": {"task": "detection", "input": (640, 640), "latency_ms": 22, "mAP": 44.2, "params_m": 26.8},
    "yolox_l_lite": {"task": "detection", "input": (640, 640), "latency_ms": 35, "mAP": 49.0, "params_m": 54.2},
    "deeplabv3plus_lite": {"task": "segmentation", "input": (512, 512), "latency_ms": 40, "mIoU": 65.3, "params_m": 11.2},
    "yoloxpose_s_lite": {"task": "pose", "input": (640, 640), "latency_ms": 30, "AP": 61.2, "params_m": 9.2},
    "efficientnet_b0_lite": {"task": "classification", "input": (224, 224), "latency_ms": 5, "top1": 76.3, "params_m": 5.3},
    "fastvit_s12": {"task": "classification", "input": (256, 256), "latency_ms": 7, "top1": 79.3, "params_m": 10.7},
    "midas_small_lite": {"task": "depth", "input": (256, 256), "latency_ms": 25, "params_m": 21.1},
}

class AM68AInferenceEngine:
    """Simulated TIDL inference for AM68A (8 TOPS)."""
    DEVICE = "AM68A"; TOPS = 8.0; RAM_GB = 8

    def __init__(self, model_name: str, simulate_latency: bool = True):
        if model_name not in SUPPORTED_MODELS:
            raise ValueError(f"Model '{model_name}' not supported on AM68A. Supported: {list(SUPPORTED_MODELS)}")
